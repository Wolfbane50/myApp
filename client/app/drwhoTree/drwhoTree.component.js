...
(function() {
  ...
  class drwhoTreeComponent {
    constructor($http, $scope, $sce, ngToast) {
        ...
        this.vidConfig = {
          sources: [{
            src: this.$sce.trustAsResourceUrl("http://127.0.0.1:9000/11%20-%203%20-%20Kd-Trees%20%282907%29.mp4"),
            type: "video/mp4"
          }],
          tracks: [],
        ...
        this.playEpisode = function(index) {
          ...
          var url = this.$sce.trustAsResourceUrl(this.selItem.SPath + "/" + this.selItem.episodes[index].fname);

        this.vidConfig.sources = [{
            src: this.$sce.trustAsResourceUrl(this.selItem.SPath + "/" + this.selItem.episodes[index].fname),
            type: this.vidTypeMap[ext]
          }];
          console.log("Set video config sources to => " + JSON.stringify(this.vidConfig.sources));
